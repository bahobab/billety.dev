...
it("returns a 401 when paying for an order that does not belog to the user", async () => {
  ...
  await request(app)
    .post("/api/payments")
    .set("Cookie", global.signin())
    .send({ orderId: order.id, token: "fghjk" })
    .expect(401);
});

it("returns a 400 when paying for a cacelled order", async () => {
  ...
  await request(app)
    ...
    .send({ orderId: order.id, token: "fdjdkjfdl" })
    .expect(400);
});
